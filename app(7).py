...
def generar_datos_simulados():
    ...
    # --- 6. SALDOS PENDIENTES (PAGOS POR COBRAR) ---
    # Simular politicas reales:
    # - Airbnb: paga ~24h despues del check-out del huesped
    # - Booking: paga a fin de mes siguiente (o con retraso de 15-30 dias)
    saldos_list = []

    # Tomar reservas completadas (check-out ya paso) que aun no se cobraron
    reservas_cobrables = df_reservas[df_reservas["Fin"] < hoy].copy()

    for idx, row in reservas_cobrables.iterrows():
        canal = row["Canal"]
        checkout = row["Fin"]
        monto = row["Precio"]

        # Simular que algunas ya se cobraron, otras no
        if random.random() < 0.4:
            estado = "Cobrado"
            fecha_cobro = checkout + timedelta(days=random.randint(1, 3))
            dias_restantes = 0
        elif random.random() < 0.3:
            estado = "En transito"
            fecha_cobro = hoy + timedelta(days=random.randint(1, 5))
            dias_restantes = (fecha_cobro - hoy).days
        else:
            estado = "Pendiente"
            if canal == "Airbnb":
                # Airbnb: paga 24h despues del check-out
                fecha_cobro = checkout + timedelta(days=1)
            else:
                # Booking: paga a fin de mes siguiente
                if checkout.day <= 15:
                    fecha_cobro = checkout.replace(day=1) + pd.DateOffset(months=1) + timedelta(days=random.randint(5, 15))
                else:
                    fecha_cobro = checkout.replace(day=1) + pd.DateOffset(months=2) + timedelta(days=random.randint(5, 15))

            # Si la fecha estimada ya paso, ajustar
            if fecha_cobro < hoy:
                fecha_cobro = hoy + timedelta(days=random.randint(2, 7))

            dias_restantes = (fecha_cobro - hoy).days

        # Comisiones simuladas (Airbnb ~3-5%, Booking ~15-18%)
        comision = 0.04 if canal == "Airbnb" else 0.16
        monto_neto = round(monto * (1 - comision), 2)
        ...
        saldos_list.append({
            ...
            "Check_out": checkout,
            "Monto_Bruto": monto,
            "Comision_%": round(comision * 100, 1),
            "Monto_Neto": monto_neto,
            "Fecha_Cobro_Est": fecha_cobro,
            "Dias_Restantes": max(0, dias_restantes),
            "Estado_Cobro": estado
        })

    df_saldos = pd.DataFrame(saldos_list)
    # Convertir fechas
    df_saldos["Check_out"] = pd.to_datetime(df_saldos["Check_out"])
    df_saldos["Fecha_Cobro_Est"] = pd.to_datetime(df_saldos["Fecha_Cobro_Est"])
...
# ============================================================
# PESTANA E: SALDOS PENDIENTES (PAGOS POR COBRAR)
# ============================================================
with tab_e:
    st.markdown('<p class="section-header">💳 Saldos Pendientes de Cobro</p>', unsafe_allow_html=True)

    # Filtrar solo pendientes y en transito
    df_saldos_activos = df_saldos[df_saldos["Estado_Cobro"].isin(["Pendiente", "En transito"])].copy()

    # Calcular totales
    total_pendiente = df_saldos_activos["Monto_Neto"].sum()
    total_airbnb = df_saldos_activos[df_saldos_activos["Canal"] == "Airbnb"]["Monto_Neto"].sum()
    total_booking = df_saldos_activos[df_saldos_activos["Canal"] == "Booking"]["Monto_Neto"].sum()

    # Proximo cobro
    if len(df_saldos_activos) > 0:
        prox_cobro = df_saldos_activos["Fecha_Cobro_Est"].min()
        dias_prox = (prox_cobro - hoy).days
    else:
        prox_cobro = None
        dias_prox = 0
    ...
    with col_s1:
        ...
        <div class="kpi-card">
            ...
            <div class="kpi-value">EUR {total_pendiente:,.0f}</div>
            <div style="font-size: 0.8rem; color: #636E72;">{len(df_saldos_activos)} reservas por cobrar</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_s2:
        st.markdown(f"""
        <div class="kpi-card">
            <div class="kpi-label">🏠 Airbnb (Pendiente)</div>
            <div class="kpi-value" style="color: {COLORS['airbnb']};">EUR {total_airbnb:,.0f}</div>
            <div style="font-size: 0.8rem; color: #636E72;">Pago ~24h post check-out</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_s3:
        st.markdown(f"""
        <div class="kpi-card">
            <div class="kpi-label">📘 Booking (Pendiente)</div>
            <div class="kpi-value" style="color: {COLORS['booking']};">EUR {total_booking:,.0f}</div>
            <div style="font-size: 0.8rem; color: #636E72;">Pago fin de mes siguiente</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_s4:
        if prox_cobro:
            st.markdown(f"""
            <div class="kpi-card">
                <div class="kpi-label">📅 Proximo Cobro Est.</div>
                <div class="kpi-value" style="font-size: 1.4rem;">{prox_cobro.strftime("%d/%m/%Y")}</div>
                <div class="{'kpi-delta-positive' if dias_prox <= 3 else 'kpi-delta-negative'}">
                    {'🟢 En ' + str(dias_prox) + ' dias' if dias_prox > 0 else '🟡 Hoy'}
                </div>
            </div>
            """, unsafe_allow_html=True)
        else:
            ...
            <div class="kpi-card">
                <div class="kpi-label">📅 Proximo Cobro Est.</div>
                <div class="kpi-value" style="font-size: 1.2rem;">Sin cobros pendientes</div>
                <div style="font-size: 0.8rem; color: #636E72;">Todo al dia</div>
            </div>
            """, unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)

    # --- GRAFICO DE SALDOS PENDIENTES POR PROPIEDAD ---
    col_graf, col_tabla = st.columns([3, 2])

    with col_graf:
        st.markdown('<p class="section-header">Saldos Pendientes por Propiedad</p>', unsafe_allow_html=True)

        if len(df_saldos_activos) > 0:
            saldos_por_piso = df_saldos_activos.groupby("Piso")["Monto_Neto"].sum().reset_index().sort_values("Monto_Neto", ascending=True)
            ...
            fig_saldos = px.bar(
                ...
                text=[f"EUR {v:,.0f}" for v in saldos_por_piso["Monto_Neto"]]
            )

            fig_saldos.update_traces(
                textposition="outside",
                hovertemplate="<b>%{y}</b><br>Pendiente: EUR %{x:,.0f}<extra></extra>"
            )
            ...
            fig_saldos.update_layout(
                ...
                paper_bgcolor="rgba(0,0,0,0)",
                font=dict(family="Inter, sans-serif", size=12),
                xaxis=dict(title="Monto Pendiente (EUR)", showgrid=True, gridcolor="rgba(0,0,0,0.05)"),
                yaxis=dict(title=""),
                coloraxis_showscale=False,
                margin=dict(l=20, r=20, t=20, b=20),
                height=350
            ...
            st.info("No hay saldos pendientes de cobro.")

    with col_tabla:
        st.markdown('<p class="section-header">Resumen por Canal</p>', unsafe_allow_html=True)

        # Tabla resumen por canal
        resumen_canal = df_saldos_activos.groupby("Canal").agg({
            "Monto_Neto": "sum",
            "Monto_Bruto": "sum",
            "Comision_%": "mean"
        }).reset_index()

        for idx, row in resumen_canal.iterrows():
            color_canal = COLORS["airbnb"] if row["Canal"] == "Airbnb" else COLORS["booking"]
            st.markdown(f"""
            <div style="background: white; border-radius: 12px; padding: 1rem; margin-bottom: 0.8rem;
                        border-left: 4px solid {color_canal}; box-shadow: 0 2px 8px rgba(0,0,0,0.03);">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.3rem;">
                    <span style="font-weight: 700; font-size: 1rem; color: {color_canal};">{row["Canal"]}</span>
                    <span style="font-size: 0.8rem; color: #636E72;">Comision ~{row["Comision_%"]:.1f}%</span>
                </div>
                <div style="font-size: 1.4rem; font-weight: 700; color: #1E3A5F;">EUR {row["Monto_Neto"]:,.0f}</div>
                <div style="font-size: 0.8rem; color: #636E72;">Bruto: EUR {row["Monto_Bruto"]:,.0f}</div>
            </div>
            """, unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)

    # --- TABLA DETALLADA DE SALDOS ---
    st.markdown('<p class="section-header">Detalle de Reservas Pendientes de Pago</p>', unsafe_allow_html=True)

    if len(df_saldos_activos) > 0:
        # Preparar DataFrame para mostrar
        df_saldos_display = df_saldos_activos[[
            "Piso", "Huesped", "Canal", "Check_out", "Monto_Bruto", 
            "Comision_%", "Monto_Neto", "Fecha_Cobro_Est", "Dias_Restantes", "Estado_Cobro"
        ]].copy()

        df_saldos_display["Check_out"] = df_saldos_display["Check_out"].dt.strftime("%d/%m/%Y")
        df_saldos_display["Fecha_Cobro_Est"] = df_saldos_display["Fecha_Cobro_Est"].dt.strftime("%d/%m/%Y")
        df_saldos_display["Monto_Bruto"] = df_saldos_display["Monto_Bruto"].apply(lambda x: f"EUR {x:,.0f}")
        df_saldos_display["Monto_Neto"] = df_saldos_display["Monto_Neto"].apply(lambda x: f"EUR {x:,.0f}")
        df_saldos_display["Comision_%"] = df_saldos_display["Comision_%"].apply(lambda x: f"{x:.1f}%")

        # Ordenar por fecha de cobro estimada (mas proximo primero)
        df_saldos_display = df_saldos_display.sort_values("Dias_Restantes", ascending=True)
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Monto_Bruto": st.column_config.TextColumn("💰 Bruto", width="small"),
                "Comision_%": st.column_config.TextColumn("📉 Comision", width="small"),
                "Monto_Neto": st.column_config.TextColumn("💵 Neto", width="small"),
                "Fecha_Cobro_Est": st.column_config.TextColumn("📅 Cobro Est.", width="small"),
                "Dias_Restantes": st.column_config.NumberColumn("⏳ Dias", width="small"),
                "Estado_Cobro": st.column_config.TextColumn("📊 Estado", width="small")
            }
        )
    else:
        st.success("✅ No hay saldos pendientes. Todas las transferencias estan al dia.")

    # --- TIMELINE DE COBROS ESPERADOS ---
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown('<p class="section-header">Linea de Tiempo de Cobros Esperados</p>', unsafe_allow_html=True)

    if len(df_saldos_activos) > 0:
        # Agrupar cobros por fecha estimada
        timeline_cobros = df_saldos_activos.groupby(["Fecha_Cobro_Est", "Canal"])["Monto_Neto"].sum().reset_index()
        timeline_cobros["Fecha_Str"] = timeline_cobros["Fecha_Cobro_Est"].dt.strftime("%d/%m")
        ...
        fig_timeline = px.bar(
            ...
            color="Canal",
            color_discrete_map={"Airbnb": COLORS["airbnb"], "Booking": COLORS["booking"]},
            text=[f"EUR {v:,.0f}" for v in timeline_cobros["Monto_Neto"]],
            barmode="group"
        )

        fig_timeline.update_traces(
            textposition="outside",
            hovertemplate="<b>%{x}</b><br>%{data.name}: EUR %{y:,.0f}<extra></extra>"
        )
        ...
        fig_timeline.update_layout(
            ...
            paper_bgcolor="rgba(0,0,0,0)",
            font=dict(family="Inter, sans-serif", size=12),
            xaxis=dict(title="Fecha estimada de cobro", showgrid=False),
            yaxis=dict(title="Monto (EUR)", showgrid=True, gridcolor="rgba(0,0,0,0.05)"),
            legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1, bgcolor="rgba(255,255,255,0.8)"),
            margin=dict(l=20, r=20, t=80, b=40),
            height=350
        )
    ...
    # --- NOTA INFORMATIVA ---
    st.markdown("""
    <div style="background: #F0F7FF; border-radius: 12px; padding: 1rem; margin-top: 1rem; border-left: 4px solid #3498DB;">
        <div style="font-weight: 600; color: #1E3A5F; margin-bottom: 0.5rem;">📋 Politicas de Pago de las Plataformas</div>
        <div style="font-size: 0.9rem; color: #636E72; line-height: 1.6;">
            <b>Airbnb:</b> Transfiere el pago del huesped aproximadamente 24 horas despues del check-in (o check-out segun el metodo). 
            Comision tipica: 3% del subtotal.<br>
            <b>Booking.com:</b> Realiza transferencias bancarias generalmente a finales del mes siguiente a la estancia. 
            Comision tipica: 15-18% sobre el total de la reserva.<br>
            <span style="color: #B2BEC3; font-size: 0.8rem;">* Las fechas mostradas son estimaciones basadas en politicas estandar. Los retrasos bancarios pueden variar.</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
