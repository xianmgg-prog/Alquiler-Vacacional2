...
"""
🏠 Dashboard de Gestión de Alquileres Vacacionales
==================================================
    ...
    for piso in pisos:
        ...
        for _ in range(num_reservas):
            ...
            reservas_list.append({
                ...
                "Estado": "Confirmada" if inicio >= hoy else "Completada",
                "Capacidad": piso["capacidad"]
            })

    df_reservas = pd.DataFrame(reservas_list)
    # Asegurar tipo datetime para usar accessor .dt
...
with tab_a:
    ...
    def kpi_card_html(label, value, delta, is_percentage=False, is_pp=False):
        delta_class = "kpi-delta-positive" if delta >= 0 else "kpi-delta-negative"
        arrow = "▲" if delta >= 0 else "▼"
        suffix = "pp" if is_pp else "%"
        unit = "" if is_percentage or is_pp else ""
        return f"""
        <div class="kpi-card">
...
with tab_c:
    ...
    # Aplicar estilos condicionales con Pandas Styler
    ...
    styled_rend = df_rend_display.style.applymap(
        color_ocupacion, subset=["Ocupacion_%"]
    ).set_properties(**{
        "text-align": "center",
        "font-size": "0.95rem"
    }).set_table_styles([
